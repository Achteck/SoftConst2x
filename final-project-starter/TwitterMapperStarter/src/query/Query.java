...
public class Query implements Observer {
    ...
    public void terminate() {
        for(MapObject mo : layer.getElements()){
    ...
    public void update(Observable o, Object arg) {
        if(filter.matches((Status) arg)){
            Image i = getUserImage((Status) arg);
            MapMarkerSimple m = new MapMarkerSimple(layer, statusCoordinate((Status) arg), color, i);
            placeMarker(m);
        }
    }

    private BufferedImage getUserImage(Status s){
        String userImgURL = s.getUser().getProfileImageURL();
        System.out.println(userImgURL);
        return ImageCache.getInstance().getImage(userImgURL);
    }

    private void placeMarker(MapMarkerSimple m){
        layer.add(m);
        map.addMapMarker(m);
    }
}
